...
app.post('/api/create-comp', (req, res) => {
	try {
		...
		competitions.set(roomId, {
			subject, language, constraints: constraints || 'None', timeLimit, createdAt: Date.now(), participants: [],
		});
...
app.post('/api/submit-code', async (req, res) => {
	try {
		...
		submissions.set(submissionId, {
			roomId, username, code, language, timestamp: Date.now(), status: 'pending',
		});
...
The following submission should ideally be written in **${competition.language}**.
If the code is in a different language, you may deduct points, but do not automatically give a score of 0. Instead, evaluate the code's correctness and provide feedback about the language mismatch.
...
{"score": "<0 to 100, where 0 is absolute failure and 100 is perfect. Do not give 0 even if the coding language is wrong, or doesn't cover all edge cases. It should only give 0 if input is empty or the code is entirely non-functional.>",
"feedback": "<A concise 2 sentence summary of the code's performance and adherence to requirements.>",
`;


			const response = await axios.post(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${GOOGLE_AI_API_KEY}`, {
				contents: [{
					parts: [{ text: prompt }],
				},],
			}, {
