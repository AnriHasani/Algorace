...
import {competitionDescriptionAtom, competitionSubjectAtom} from "../components/jotai/competitionAtoms.ts";


const CompetitionLobbyPage = () => {
  const [subject] = useAtom(competitionSubjectAtom);
  const [description] = useAtom(competitionDescriptionAtom);

  const { roomId } = useParams<{ roomId: string }>();
  const [username, setUsername] = useState('');
  const [hasJoined, setHasJoined] = useState(false);
  const [copied, setCopied] = useState(false);
  const { joinCompetition, loading, startTimer } = useCompetition();
                  ...
                  <div className="space-y-6">
                    <div className="text-center mb-6">
                      ...
                      <p className="text-gray-400 mt-1">
                        You've successfully joined the competition as{' '}
                        <span className="text-primary-400 font-medium">{username}</span>
                      </p>
                    </div>
